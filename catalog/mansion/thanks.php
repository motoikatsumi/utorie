...
//$_address ="ccc@channnana.site";
$_address ="info@utorie.jp";
...
$refrom_name = "CACHIE（カチエ）｜女性専用24時間フィットネスジム";

// 差出人に送信確認メールを送る場合のメールのタイトル（上記で1を設定した場合のみ）
$re_subject = "一括資料請求 受付｜utorie（ウトリエ）";
...
$remail_text = <<< TEXT
一括資料請求ありがとうございます。以下の内容で一括資料請求を受け付けました。
...
function postToMail($arr){
	...
	foreach($arr as $key => $val){
		...
		if($out != "confirm_submit" && $key != "httpReferer" && $key != "upfilePath" && $key != "upfileType") {
			...
			}else if($key == "howbigland"){
				$key = "希望の広さは？";
			}else if($key == "howbigland_value"){
				continue;
			}else if($key == "budget"){
				$key = "ご希望の予算は？";
			}else if($key == "name"){
